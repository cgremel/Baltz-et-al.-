...
%GCAMP.baselien_delta_f_First_HE_After_RE =( delta_F_First_HE_After_RE - delta_F_Pre_LP_ON_Baselines_Mean(delta_F_Pre_First_HE_After_RE_Baselines_Idx) ./
...
GCAMP.delta_F_First_HE_After_Fail= delta_F_First_HE_After_Fail;

% %% For each head entry, extract peri-event (first head entry after press) GCAMP traces
%     
% for head_entry = 1:length(HE_ON_timestamps)
%     % Find indices of first head entry after reward timestamps closes to GCAMP trace
%     % timestamps
%     Closest_HE_ON_idx = nearestpoint(HE_ON_timestamps(head_entry),GCAMP.gcampdata_timestamps);
%     % Find index of the lever press that led to this head entry
%     Closest_LP_before_HE_ON_idx = nearestpoint(First_HE_After_Fail_timestamps(head_entry),GCAMP.LP_ON_timestamps,'previous');
%     LP_before_HE_ON_timestamp = GCAMP.LP_ON_timestamps(Closest_LP_before_HE_ON_idx);
%     LP_Idx_before_HE_ON_timestamp = nearestpoint(LP_before_HE_ON_timestamp,GCAMP.gcampdata_timestamps);
...
% end
% % Normalize peri-event GCAMP traces to pre-lever press onset baselines
% GCAMP.baseline_norm_HE = (delta_F_First_HE_After_Fail - delta_F_Pre_LP_ON_Baselines_Mean(delta_F_First_HE_After_Fail_Baselines_Idx)) ./ delta_F_Pre_LP_ON_Baselines_STD(delta_F_First_HE_After_Fail_Baselines_Idx);
% 
...
%             interp_data(lever_press,:) = interp1(t0,duration_activity,t1,'makikma'); %data interpolated to fixed sample space
             interp_data_norm(lever_press,:) = interp1(t0,duration_activity_norm,t1,'makima'); %norm data interpolated to fixed sample space
            interp_data(lever_press,:) = interp1(t0,duration_activity,t1,'makima'); %data interpolated to fixed sample space
        end
    end
