...
%% Saves GCAMP and Behavior data into a data structure, edited 6.28.22 by ETB to include licks
...
% Subtract logicals from each other in Lever Press column
 ...
 %% Branch 3.1.22: to remove LP that happen before baseline is established
...
if (LP_ON_timestamps(1) - GCAMP.gcampdata_timestamps(1))/1000 < abs(GCAMP.base_time_start)
    GCAMP.beh_data =  GCAMP.beh_data(LP_OFF_index(1):end,:);
    'removing behavioral lp timestamps to have proper baseline' % make sure that you know that the first LP that happens 
end

%% Branch 6.7.22: to remove HE that happen before baseline is established
...
if (HE_ON_timestamps(1) - GCAMP.gcampdata_timestamps(1))/1000 < abs(GCAMP.base_time_start)
    GCAMP.beh_data =  GCAMP.beh_data(HE_OFF_index(1):end,:);
    'removing behavioral he timestamps to have proper baseline' % make sure that you know that the first LP that happens 
end

% %% Branch 6.28.22: to remove licks that happen before baseline is
% % %%established
% switches_lick = diff(GCAMP.beh_data(:,5)); %3 %OFC paper
...
% 
% if (lick_ON_timestamps(1) - GCAMP.gcampdata_timestamps(1))/1000 < abs(GCAMP.base_time_start)
%     GCAMP.beh_data =  GCAMP.beh_data(lick_OFF_index(1):end,:);
%     'removing behavioral licks timestamps to have proper baseline' % make sure that you know that the first LP that happens 
% end
...
% lick_time =(lick_All(:,2)-lick_All(:,1))-20; 
% lick_idx= find(lick_time > 0); 
...
%  %% First Lick after Rein delivery
% %Find the first lick made after a Reinforcer
% First_lick_After_RE_idx =[];
% for k = 1:length(RE_ON_timestamps) %for each RE_ON timestamp
%     % Find index of this LP_ON timestamp in the vector of all of your
%     % timestamps
%     Closest_lick_After_RE_idx = nearestpoint(RE_ON_timestamps(k),lick_ON_timestamps(lick_idx),'next');
%     First_lick_After_RE_idx = [First_lick_After_RE_idx; Closest_HE_After_RE_idx];
%     % First_HE_After_RE_idx(isnan(First_HE_After_RE_idx)) = [];
% end
% % Remove NaNs (HE happening outside of last LP)
% First_lick_After_RE_idx(isnan(First_lick_After_RE_idx)) = [];
% % Remove instances in which the same head entry happens after two or more
% % reinforcements to avoid sampling duplicate traces
% First_lick_After_RE_idx = unique(First_lick_After_RE_idx,'rows');
% % Remove rare instances in which nearestpoint captures a head entry before
% % a lever press
% remove_idx = find(lick_ON_timestamps(First_lick_After_RE_idx) <= min(LP_ON_timestamps));
% First_lick_After_RE_idx(remove_idx) = [];
...
% GCAMP.lick_ON_timestamps = lick_ON_timestamps;
% GCAMP.lick_OFF_timestamps = lick_OFF_timestamps;
